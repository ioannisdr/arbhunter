...
export default function Dashboard() {
  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-white font-sans p-8 select-none">
      <nav className="mb-12 flex justify-between items-center border-b border-white/5 pb-6">
        ...
        <div className="xl:col-span-2">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold flex items-center gap-3">
              Live Opportunities
              <span className="bg-emerald-500/20 text-emerald-500 text-sm py-1 px-3 rounded-full font-medium">
                {discrepancies.length} Active
              </span>
            </h1>
            <div className="text-neutral-500 text-sm flex items-center gap-2">
              <RefreshCw size={14} className="animate-spin" /> Auto-syncing
            </div>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div 
                        ...
                        className="p-6 hover:bg-neutral-800/80 transition-colors cursor-pointer flex justify-between items-center group"
                      >
                        <div>
                          ...
                          <div className="flex items-center gap-3 text-neutral-400 text-sm mb-2">
                            {arb.league && <span className="bg-white/10 px-2 py-0.5 rounded text-neutral-300">{arb.league}</span>}
                            <span className="flex items-center gap-1"><Calendar size={14}/> {matchDate}</span>
                          </div>
                        ...
                        <div className="flex gap-6">
                          {arb.details.map((det: any, idx: number) => (
                            <div key={idx} className="text-center px-4 border-l border-white/5 first:border-0">
                              <div className="text-xs text-neutral-500 uppercase tracking-wider mb-1">{det.selection} <br/><span className="opacity-70 text-[10px]">{det.bookie}</span></div>
                              <div className="font-mono text-lg font-bold text-white">{det.price.toFixed(2)}</div>
                            </div>
                          ))}
                        </div>

                        <div className="text-right pl-6 border-l border-white/10 flex items-center gap-6">
                          <div>
                          ...
                          >
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                              {stakes.map((stake: any, idx: number) => (
                                <div key={idx} className="bg-neutral-900 rounded-xl p-4 border border-white/5 relative overflow-hidden">
                                  <div className="absolute top-0 left-0 w-1 h-full bg-neutral-700"></div>
                                  <div className="flex justify-between items-start mb-4">
                                    <div>
                                      <div className="text-sm text-neutral-400">Place bet on</div>
                                      <div className="font-bold text-emerald-400">{stake.bookie}</div>
                                    </div>
                                    <div className="text-right">
                                      <div className="text-xs text-neutral-500">Selection</div>
                                      <div className="font-bold">{stake.selection} @ {stake.price.toFixed(2)}</div>
                                    </div>
                                  </div>
                                  <div className="text-center pt-4 border-t border-white/5">
                                    <div className="text-xs text-neutral-500 uppercase tracking-wider mb-1">Stake Required</div>
                                    <div className="font-mono text-2xl font-bold text-white">{formatSEK(stake.stake)}</div>
                                  </div>
                                </div>
                              ))}
                            </div>
                            ...
                            <div className="flex justify-between items-center bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4">
                              <div className="text-emerald-500 font-medium">Guaranteed Payout regardless of outcome:</div>
                              <div className="text-right flex items-baseline gap-2">
                                <span className="font-mono text-2xl font-bold text-emerald-400">{formatSEK(stakes[0].payout)}</span>
                                <span className="text-emerald-500/50 text-sm font-medium">({formatSEK(currentStakePerBet)} total stake)</span>
                              </div>
                            </div>
                          </motion.div>
                        )}
