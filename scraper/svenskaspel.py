...
class SvenskaSpelScraper:
    def __init__(self):
        self.sportsbook = "Svenska Spel"
    ...
    async def fetch_matches(self):
        ...
        async with httpx.AsyncClient() as client:
            for sport_name, endpoint in self.sports_endpoints.items():
                url = f"https://eu-offering-api.kambicdn.com/offering/v2018/sve/listView/{endpoint}?lang=sv_SE&market=SE"
                try:
...
if __name__ == "__main__":
    async def run():
        s = SvenskaSpelScraper()
        matches = await s.fetch_matches()
        if matches:
            print(f"First match: {matches[0].team_home} vs {matches[0].team_away} with {len(matches[0].odds)} odds.")
    asyncio.run(run())
